...
def run_publish(
    ...
    # 2. 检查状态
    if current not in (TaskStatus.PR_READY, TaskStatus.MERGING, TaskStatus.TAGGING, TaskStatus.RELEASING, TaskStatus.PUBLISH_INCOMPLETE):
        result["steps"].append({
            "step": "check_state",
            "status": "error",
    ...
    # 5. 状态转换: PR_READY → MERGING → TAGGING
    try:
    ...
    try:
        tag_name = version
        commit_sha = git.get_head_sha()
        git.create_annotated_tag(tag_name, f"Release {version}")
        result["tag"] = tag_name
        result["steps"].append({
            "step": "create_tag",
            "status": "ok",
            "message": f"Git 标签已创建: {tag_name}",
        })
    except Exception as e:
        result["steps"].append({
            "step": "create_tag",
            "status": "error",
            "message": f"创建 Git 标签失败: {e}",
        })
        sm.transition(TaskStatus.PUBLISH_INCOMPLETE)
        _output(result, json_output)
        return False

    # 7. 状态转换: TAGGING → RELEASING
    try:
    ...
    try:
        ...
        result["steps"].append({
            "step": "create_release",
            "status": "warn",
            "message": f"创建 GitHub Release 失败（标签已创建）: {e}",
        })

    # 9. 状态转换: RELEASING → HANDOFF_UPDATING → CLEANING → COMPLETE
    try:
